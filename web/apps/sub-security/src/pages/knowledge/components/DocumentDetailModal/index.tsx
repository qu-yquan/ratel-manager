...
const EVENT_LABELS: Record<string, string> = {
  UPLOADED: "加入知识库",
  PARSE_SUCCEEDED: "文件解析成功",
  INGEST_SUCCEEDED: "导入成功",
  INGEST_FAILED: "导入失败",
  RETRY_SUBMITTED: "重新导入",
  MARKDOWN_EDITED: "Markdown 编辑",
  DOCUMENT_ENABLED: "文档启用",
  DOCUMENT_DISABLED: "文档禁用",
};
const DocumentDetailModal: React.FC<Props> = ({
  documentId,
  canEdit,
  onClose,
  onUpdated,
  ...
  const [blockData, setBlockData] = useState<KnowledgeDocumentBlocks>({
    blocks: [],
  });
  const [draftBlocks, setDraftBlocks] = useState<KnowledgeDocumentBlocks>({
    blocks: [],
  });
  const [blockTypes, setBlockTypes] = useState<
    Array<{ label: string; value: KnowledgeBlockType }>
  >([]);
    ...
    const poll = async () => {
      try {
        ...
        const [logs, content, blocks] = await Promise.allSettled([
          getEvents(documentId),
          detail.documentStatus === "PROCESSED"
            ? getMarkdown(documentId)
        ...
        if (blocks.status === "fulfilled" && blocks.value !== undefined)
          setBlockData(blocks.value);
        setDocument(detail);
        if (
          detail.documentStatus !== "UPLOADED" &&
          ...
          },
          {
            ...
            children: (
              <Descriptions bordered column={2} size="small">
                ...
                <Descriptions.Item label="是否向量化">
                  {document?.embeddingCompleted ? "是" : "否"}
                </Descriptions.Item>
                <Descriptions.Item label="图片 OCR">
                  {document?.imageOcrParsed ? "是" : "否"}
                </Descriptions.Item>
                    ...
                    {editing ? (
                      ...
                      blockData.blocks.length > 0 && (
                        <Button
                          ...
                          onClick={() => {
                            setDraftBlocks({
                              pageVersionId: blockData.pageVersionId,
                ...
                {editing ? (
                  <>
                    <Alert
                      ...
                      message="按 Block 编辑已有内容和类型，保存后将重建检索索引。"
                    />
                    <div className={styles.blockList}>
                      {draftBlocks.blocks.map((block, index) => (
                ...
                items={events.map((event) => ({
                  ...
                  children: (
                    <div>
                      ...
                      <div>
                        <Typography.Text type="secondary">
                          {event.occurredAt} · {event.createOp ?? "系统"}
                        </Typography.Text>
                      </div>
