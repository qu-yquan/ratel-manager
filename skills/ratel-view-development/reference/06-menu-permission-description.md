...
2. 生成可执行的 `security_menu` `INSERT` 语句，并在最终交付结果中输出，方便用户自行维护菜单数据。
3. **禁止将生成的菜单脚本写入、追加或更新到项目内任何 SQL 文件**，包括初始化数据、数据库迁移、升级和测试 SQL 文件；除非用户在当前任务中明确要求修改指定 SQL 文件。
...
- 菜单 SQL 仅作为最终交付内容输出，不得落入仓库中的任何 SQL 文件。
...
菜单 SQL 必须直接放在最终回复的 SQL 代码块中，仅供用户复制执行；不得以新增或修改项目 SQL 文件的方式交付。
